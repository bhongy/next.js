...
function externalsConfig (isServer) {
  const externals = []

  // for client: don't use any externals
  // so all node_modules will be compiled into the output bundles
  // so that the bundles have 0 runtime dependencies
  if (!isServer) {
    return externals
  }
...
function optimizationConfig ({ dev, isServer, totalPages }) {
  if (isServer) {
    // don't use terser (minify) or splitchunk for server
    return {
      splitChunks: false,
      minimize: false
    }
  ...
  const config = {
    runtimeChunk: {
      // single runtime chunk to share for all chunks
...
export default async function getBaseWebpackConfig (
  dir,
  { dev = false, isServer = false, buildId, config, entrypoints }
) {
  ...
  let webpackConfig = {
    ...
    externals: externalsConfig(isServer),
